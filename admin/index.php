...
<tr><td><strong><?= lpk_h($a['title']) ?></strong><small>/guides/<?= lpk_h($a['slug']) ?>/</small></td><td><?= lpk_h($a['category']) ?></td><td><span class="status status-<?= lpk_h($a['status']) ?>"><?= strtoupper(lpk_h($a['status'])) ?></span></td><td><?= lpk_h(substr((string)$a['updated_at'],0,10)) ?></td><td class="admin-actions"><a href="/admin/edit.php?id=<?= rawurlencode($a['id']) ?>">Edit</a><a href="/admin/preview.php?id=<?= rawurlencode($a['id']) ?>" target="_blank" rel="noopener">Preview</a><?php if (($a['status'] ?? '') === 'published'): ?><a href="/guides/<?= lpk_h($a['slug']) ?>/" target="_blank" rel="noopener">View</a><?php endif; ?></td></tr>
<?php endforeach; ?>
</tbody></table></div>
<div class="admin-help"><h2>How publishing works</h2><p>Drafts stay private. Publishing writes a real static HTML page at <code>/guides/your-slug/index.html</code>, refreshes the Guides hub, and updates <code>sitemap-articles.xml</code>. Your existing piano pages stay untouched.</p></div>
