  ...
  if (!$errors) {
    ...
    if (!$errors) {
      ...
      try {
        ...
        else { lpk_regenerate_guides_hub(); lpk_regenerate_article_sitemap(); }
      } catch (Throwable $e) { $errors[] = $e->getMessage(); }
      if (!$errors) {
...
<div class="editor-main">
  <section class="admin-panel"><label>Article title<input id="articleTitle" name="title" required maxlength="140" value="<?= lpk_h($a['title']) ?>" placeholder="Piano Scale Fingering: A Beginner Guide"></label><label>URL slug<div class="slug-row"><span>/guides/</span><input id="articleSlug" name="slug" required maxlength="100" value="<?= lpk_h($a['slug']) ?>"><span>/</span></div></label></section>
...
<aside class="editor-side">
  ...
  <section class="admin-panel"><h2>Search preview</h2><label>SEO title <input name="seo_title" id="seoTitle" maxlength="70" value="<?= lpk_h($a['seo_title']) ?>"></label><div class="char-count" data-count-for="seoTitle"></div><label>Meta description<textarea name="meta_description" id="metaDescription" rows="4" maxlength="170"><?= lpk_h($a['meta_description']) ?></textarea></label><div class="char-count" data-count-for="metaDescription"></div><div class="serp-preview"><span class="serp-site">Learn Piano Keys</span><strong id="serpTitle"><?= lpk_h($a['seo_title'] ?: $a['title']) ?></strong><span id="serpUrl">learnpianokeys.com/guides/<?= lpk_h($a['slug']) ?>/</span><p id="serpDesc"><?= lpk_h($a['meta_description']) ?></p></div></section>
  <section class="admin-panel"><h2>Article details</h2><label>Category<select name="category"><?php foreach (lpk_article_categories() as $c): ?><option <?= $a['category']===$c?'selected':'' ?>><?= lpk_h($c) ?></option><?php endforeach; ?></select></label><label>Short excerpt<textarea name="excerpt" rows="4" maxlength="260" placeholder="Shown on the Guides page and related cards."><?= lpk_h($a['excerpt']) ?></textarea></label><label>Featured image <input type="file" name="featured_image" accept="image/jpeg,image/png,image/webp"></label><?php if (!empty($a['featured_image'])): ?><img class="admin-thumb" src="<?= lpk_h($a['featured_image']) ?>" alt="Current featured image"><small><?= lpk_h($a['featured_image']) ?></small><?php endif; ?></section>
</aside>
</form>
